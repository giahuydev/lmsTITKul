    ...
    <div className="max-w-4xl mx-auto pb-12 space-y-6">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center space-x-4">
          <div className="w-14 h-14 bg-indigo-100 rounded-2xl flex items-center justify-center border-2 border-indigo-200">
        ...
        <button className="text-sm font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-4 py-2 rounded-xl transition-colors flex items-center">
      ...
      <div className="space-y-4">
        {notifications.length > 0 ? notifications.map((noti) => (
          <div key={noti.id} className={`bg-white border rounded-2xl p-5 flex items-start transition-colors relative overflow-hidden ${
            noti.read ? 'border-slate-200' : 'border-indigo-300 shadow-sm bg-indigo-50/30'
          }`}>
             {!noti.read && (
             ...
             <div className="flex-1 ml-2">
               <div className="flex items-center space-x-2 mb-2">
                  {noti.pinned && <span className="bg-red-100 text-red-700 text-xs font-bold px-2 py-0.5 rounded-md flex items-center"><img src="https://img.icons8.com/color/48/pin.png" className="w-3 h-3 mr-1" alt="pin" /> Ghim</span>}
             ...
             {!noti.read && (
                <div className="w-3 h-3 bg-indigo-500 rounded-full shrink-0 ml-4 mt-2"></div>
             )}
          </div>
        )) : (
