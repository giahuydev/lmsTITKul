...
export function TableHeader({ className, ...props }: React.HTMLAttributes<HTMLTableSectionElement>) {
  return <thead className={cn("bg-slate-50/80 border-b border-slate-200", className)} {...props} />;
}
...
export function TableHead({ className, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      className={cn("h-12 px-4 text-left align-middle font-medium text-slate-600 [&:has([role=checkbox])]:pr-0", className)}
      {...props}
    />
  );
}
